...
class QtAppTests(unittest.TestCase):
    ...
    def test_qt_window_constructs(self) -> None:
        ...
        try:
            ...
            self.assertEqual(window.service_status.text(), "Service belum terhubung")
            self.assertEqual(window.hero_title.text(), "Dasbor Bot")
            self.assertEqual(window.nav_group.title(), "Navigasi")
            ...
            self.assertEqual(window.tabs.tabText(0), "Feed Bot")
    ...
    def test_qt_accessibility_defaults_expose_names_and_tab_flow(self) -> None:
        ...
        try:
            ...
            self.assertIn("dependency operator", window.gate_primary_button.accessibleDescription())
            self.assertEqual(window.gate_retry_button.accessibleName(), "Coba lagi pemeriksaan dependency")
            self.assertIn("Jalankan ulang", window.gate_retry_button.accessibleDescription())
            self.assertEqual(window.gate_dev_button.accessibleName(), "Masuk mode dev")
            self.assertIn("mode dev", window.gate_dev_button.accessibleDescription())
            self.assertEqual(window.codex_command_input.accessibleName(), "Command AI runtime")
            self.assertIn("executable", window.codex_command_input.accessibleDescription())
            self.assertEqual(window.codex_cwd_input.accessibleName(), "Workspace AI")
            self.assertIn("workspace utama", window.codex_cwd_input.accessibleDescription())
            self.assertEqual(window.ai_documents_input.accessibleName(), "Dokumen AI")
            self.assertIn("dokumen referensi", window.ai_documents_input.accessibleDescription())
            self.assertEqual(window.ai_context_input.accessibleName(), "Folder context AI")
            self.assertIn("resume state", window.ai_context_input.accessibleDescription())
            self.assertEqual(window.timeout_input.accessibleName(), "Timeout AI runtime")
            self.assertIn("detik", window.timeout_input.accessibleDescription())
            self.assertEqual(window.check_mt5_button.accessibleName(), "Cek MT5")
            self.assertIn("terminal", window.check_mt5_button.accessibleDescription())
            self.assertEqual(window.load_codex_button.accessibleName(), "Cek AI runtime")
            self.assertIn("workspace", window.load_codex_button.accessibleDescription())
            self.assertEqual(window.refresh_button.accessibleName(), "Refresh data")
            self.assertIn("snapshot market", window.refresh_button.accessibleDescription())
