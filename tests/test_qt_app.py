...
class QtAppTests(unittest.TestCase):
    ...
    def test_qt_window_constructs(self) -> None:
        ...
        try:
            ...
            self.assertIs(window.strategy_page.layout().itemAt(2).widget(), window.trade_control_scroll)
            self.assertIs(window.dashboard_page.layout().itemAt(2).widget(), window.snapshot_dashboard)
            self.assertIs(window.logs_page.layout().itemAt(0).widget(), window.logs_group)
