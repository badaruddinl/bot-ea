...
class GoldMCiContractTests(unittest.TestCase):
    def test_release_workflow_cannot_omit_new_tests(self) -> None:
        workflow = (
            Path(__file__).resolve().parents[1] / ".github" / "workflows" / "goldm-core-v2.yml"
        ).read_text(encoding="utf-8")
        self.assertIn("python -m pytest -q", workflow)
        self.assertIn("--basetemp=.ci-pytest-temp", workflow)
        self.assertIn("--junitxml=.ci-evidence\\pytest-junit.xml", workflow)
        self.assertIn("python scripts/quality_gate.py --base $base --head HEAD", workflow)
