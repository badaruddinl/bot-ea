...
SCRIPT = Path(__file__).resolve().parents[2] / "scripts" / "verify_g13_compile.py"


def test_g13_compile_verifier_is_profile_complete_and_harness_fail_closed() -> None:
    value = SCRIPT.read_text(encoding="utf-8")
    assert "verify_g11_compile_artifacts" in value
    assert '"gate": "G13"' in value
    assert '"status": "COMPILE_PASS"' in value
    assert '"production_real_orders": "DISABLED"' in value
    assert "GoldEngineBearParityHarness.mq5" in value
