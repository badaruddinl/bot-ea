...
def bind_validation_env(monkeypatch) -> None:
    ...
    monkeypatch.setenv("GOLDM_REAL_MT5_LOGIN", "654321")
    monkeypatch.setenv("GOLDM_REAL_MT5_SERVER", "GOLDM-REAL")


def test_goldi_shadow_and_guarded_demo_configs_are_separate_and_profile_bound(
    monkeypatch,
) -> None:
    bind_validation_env(monkeypatch)
    root = REPOSITORY_ROOT / "config" / "validation" / "goldi"
    shadow = load_worker_config(root / "worker-shadow.json")
    demo = load_worker_config(root / "worker-demo.json")

    assert shadow.group == demo.group == "goldi"
...
def test_goldm_read_only_config_reuses_identity_but_never_order_authority(
    monkeypatch,
) -> None:
    bind_validation_env(monkeypatch)
    validation = load_worker_config(
        REPOSITORY_ROOT / "config" / "validation" / "goldm" / "worker-read-only.json"
    )
    ...
    assert validation.execution_mode == "signal_only"
    assert validation.orders_enabled is False
...
def test_g10_probe_is_read_only_and_preflight_never_persists_bindings(monkeypatch) -> None:
    ...
    sentinel_server = "PRIVATE-REAL-SERVER-SENTINEL"
    monkeypatch.setenv("GOLDM_REAL_MT5_LOGIN", sentinel_login)
    monkeypatch.setenv("GOLDM_REAL_MT5_SERVER", sentinel_server)
