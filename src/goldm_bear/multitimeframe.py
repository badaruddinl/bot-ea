...
from .candidate import confluence_v1_config
from .engine import (
    BearBar,
    BearDecision,
    BearEngine,
...
class BearMultiTimeframeReplay:
    ...
    def run(
        ...
        *,
        ...
        for setup in setup_signals:
            ...
            h1_history = h1_bars[
                max(0, h1_index - self.config.h1_sma_period - 2) : h1_index
            ]
            ...
            m5_result = self._arm_on_m5(
                ...
                m5_bars[
                    validation_start : validation_start + self.config.m5_watch_bars
                ],
                setup_available,
            )
            ...
            if entry_plan is None:
                m1_cancelled += 1
                continue
            opened_index = bisect.bisect_left(m1_times, entry_plan["opened_at"])
    ...
    def _arm_on_m5(
        ...
        for index, bar in enumerate(candidates):
            context = history + candidates[: index + 1]
            if len(context) < 15:
                continue
            atr = _average_true_range(context, 14)
            tolerance = max(self.config.spread_floor, atr * 0.20)
            acceptance = (
                index + 1 >= self.config.m5_acceptance_closes
                and all(
                    item.close > resistance + tolerance
                    for item in candidates[
                        index + 1 - self.config.m5_acceptance_closes : index + 1
                    ]
                )
            )
            if acceptance:
            ...
            if not touched:
                ...
                if (
                    retreated
                    and index - last_touch >= self.config.m5_touch_separation_bars
                ):
                    touches += 1
            ...
            close_location = (
                (bar.close - bar.low) / bar.full_range
                if bar.full_range > 0
                else 1.0
            )
            momentum = bool(
            ...
            strong_failure = bool(
                momentum
                and bar.body >= atr * 0.55
                and bar.high >= resistance - tolerance
            )
            repeated = bool(
    ...
    def _resolve_m1(
        ...
        last_close = entry
        for bar in bars:
            if bar.time < plan["opened_at"] or bar.time >= to_time:
                continue
        ...
        return BearV4Outcome(
            ...
            mfe_r=mfe_r,
            mae_r=mae_r,
            m5_touches=int(plan["m5_touches"]),
            m5_rejections=int(plan["m5_rejections"]),
            m1_touches=int(plan["m1_touches"]),
