...
class _ConfluenceDecisionFields(TypedDict):
    confluence_votes: int
    fibonacci_zone_low: float | None
    fibonacci_zone_high: float | None
    ...
    supply_proximal: float | None
    supply_distal: float | None
    supply_retest: bool
    momentum_restart: bool
    exhausted: bool


class BearAction(str, Enum):  # noqa: UP042 - preserve legacy str(Enum) semantics
    WAIT = "WAIT"
    WATCH = "WATCH"
    SELL = "SELL"


class BearExitAction(str, Enum):  # noqa: UP042 - preserve legacy str(Enum) semantics
    HOLD = "HOLD"
...
class BearEngineConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0 < self.fibonacci_retracement_low < self.fibonacci_retracement_high < 1):
...
class BearEngine:
    ...
    def minimum_bars(self) -> int:
        ...
        if self.config.higher_timeframe_filter_enabled:
            lookbacks.append((self.config.h1_sma_period + self.config.h1_slope_lookback + 2) * 4)
    ...
    def evaluate(self, bars: Sequence[BearBar]) -> BearDecision:
        ...
        if self.config.higher_timeframe_filter_enabled and not bool(h1_trend["bearish"]):
            return self._wait(
                latest,
        ...
        confirmed_failure = (
            latest.close < bars[-2].low and latest.close < latest.open and latest.body >= 0.65 * atr
        )
        ...
        if chase_distance > self.config.maximum_chase_atr * atr and (
            not confirmed_failure
            or chase_distance > self.config.maximum_confirmed_failure_chase_atr * atr
        ):
            return self._wait(
                latest,
        ...
        if (
            ...
            return BearDecision(
                ...
                resistance_touch_count=_as_int(resistance_range["touches"]),
                resistance_rejection_count=_as_int(resistance_range["rejections"]),
        ...
        if (
            (reward < required_room_atr * atr or reward_risk < required_reward_risk)
            and strong_failure
            and len(targets) > 1
        ):
            ...
            if (
                ...
                and continuation_reward_risk >= self.config.minimum_continuation_reward_risk
            ):
        ...
        return BearDecision(
            ...
            stop=stop,
            take_profit=take_profit,
            take_profit_2=(targets[1] if not continuation_target and len(targets) > 1 else None),
            reward_risk=reward_risk,
            regime_slope_atr=slope,
            regime_drop_atr=regime_drop,
            chase_distance_atr=chase_distance / atr,
            ...
            h1_close=_optional_float(h1_trend["close"]),
            h1_sma=_optional_float(h1_trend["sma"]),
            h1_sma_slope=_optional_float(h1_trend["slope"]),
            resistance_touch_count=_as_int(resistance_range["touches"]),
            resistance_rejection_count=_as_int(resistance_range["rejections"]),
            resistance_acceptance=bool(resistance_range["acceptance"]),
            **self._confluence_decision_fields(confluence),
    ...
    def evaluate_exit(
        ...
        strong_break = (
            latest.close > invalidation and latest.close > latest.open and latest.body >= 0.5 * atr
        )
    ...
    def _resistance_levels(
        ...
        levels = [_Level(price, "swing") for price in _swing_highs(history, self.config.swing_span)]
    ...
    def _support_levels(self, history: Sequence[BearBar], entry: float) -> list[_Level]:
        ...
        lows.extend(
            level for level in self._psychological_levels(lower_bound, entry) if level.price < entry
        )
        return sorted(
    ...
    def _resistance_range_stats(
        ...
        for index, bar in enumerate(recent):
            ...
            if not touched:
                if bar.close <= resistance - retreat_distance:
                    retreated = True
                continue
            if retreated and index - last_touch >= self.config.resistance_touch_separation_bars:
                touches += 1
                if bar.close < resistance and (bar.close < bar.open or bar.upper_wick >= bar.body):
                    rejections += 1
                last_touch = index
                retreated = False
        ...
        three_of_four = bool(len(outside) >= 4 and sum(outside[-4:]) >= 3)
    ...
    def _h1_trend_stats(self, bars: Sequence[BearBar]) -> dict[str, object]:
        ...
        slope = _linear_slope(moving_averages[-self.config.h1_slope_lookback :])
    ...
    @staticmethod
    def _confluence_decision_fields(
        stats: dict[str, object],
    ) -> _ConfluenceDecisionFields:
        return {
            "confluence_votes": _as_int(stats["votes"]),
            "fibonacci_zone_low": _optional_float(stats["fibonacci_zone_low"]),
            "fibonacci_zone_high": _optional_float(stats["fibonacci_zone_high"]),
            "fibonacci_retest": bool(stats["fibonacci_retest"]),
            "rsi_value": _optional_float(stats["rsi_value"]),
            "rsi_turn_down": bool(stats["rsi_turn_down"]),
            "stochastic_k": _optional_float(stats["stochastic_k"]),
            "stochastic_d": _optional_float(stats["stochastic_d"]),
            "stochastic_turn_down": bool(stats["stochastic_turn_down"]),
            "supply_proximal": _optional_float(stats["supply_proximal"]),
            "supply_distal": _optional_float(stats["supply_distal"]),
    ...
    def _confluence_stats(
        ...
        fibonacci_zone_high = None
        fibonacci_retest = False
        if impulse_low is not None and best_drop >= atr * self.config.fibonacci_min_impulse_atr:
            fibonacci_zone_low = impulse_low + best_drop * self.config.fibonacci_retracement_low
            fibonacci_zone_high = impulse_low + best_drop * self.config.fibonacci_retracement_high
            fibonacci_retest = bool(
                any(
        ...
        if supply_proximal is not None and supply_distal is not None:
            supply_retest = bool(
                any(
                    bar.high >= supply_proximal - tolerance and bar.low <= supply_distal + tolerance
                    for bar in bars[-3:]
                )
        ...
        close_location = (
            (latest.close - latest.low) / latest.full_range if latest.full_range > 0 else 1.0
        )
        momentum_restart = bool(
            latest.close < latest.open
            and close_location <= 0.40
            and (latest.close < previous.low or latest.body >= atr * self.config.momentum_body_atr)
        )
    ...
    def _score(self, *, slope: float, regime_drop: float, rejection: bool) -> int:
        ...
        return round(min(100.0, 15.0 + slope_strength + drop_strength + rejection_strength))
...
def _stochastic_stats(
    ...
    for end in range(len(bars) - smoothing, len(bars) + 1):
        ...
        values.append(
            50.0 if denominator <= 0 else (bars[end - 1].close - lowest) / denominator * 100.0
        )
    return {
        "k": values[-1],
...
def _swing_highs(bars: Sequence[BearBar], span: int) -> list[float]:
    ...
    for index in range(span, len(bars) - span):
        ...
        neighbours = tuple(bars[index - span : index]) + tuple(bars[index + 1 : index + span + 1])
